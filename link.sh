...
# Flutter Documentation and Configuration Linker
# Creates symlinks to centralized documentation and configuration files
# Usage: /opt/flutter/flutter-docs/link.sh
...
echo -e "${BLUE}Flutter Documentation and Configuration Linker${NC}"
echo -e "${BLUE}===============================================${NC}"
echo ""
...
check_target_directory() {
    ...
    if [[ "$TARGET_DIR" == "$SOURCE_DIR" ]]; then
        ...
        echo -e "  Please run this script from your Flutter project directory"
        exit 1
    fi

    # Check if we're in a reasonable location (not within flutter-docs)
    if [[ "$TARGET_DIR" == "/opt/flutter/flutter-docs"* ]]; then
        echo -e "${RED}✗ Error: Cannot create symlinks within the flutter-docs directory${NC}"
        echo -e "  Please run this script from your Flutter project directory"
        exit 1
    fi

    # Warn if not under /opt/flutter/ or common Flutter project locations
    if [[ "$TARGET_DIR" != "/opt/flutter/"* ]] && [[ "$TARGET_DIR" != "$HOME/"* ]] && [[ "$TARGET_DIR" != "/home/"* ]]; then
        echo -e "${YELLOW}⚠️  Warning: You're not in a standard Flutter project directory${NC}"
...
update_gitignore() {
    ...
    if [[ ${#missing_entries[@]} -gt 0 ]]; then
        ...
        echo "# Flutter Documentation (symlinked - root-relative paths only)" > "$temp_file"
...
echo -e "  📁 ${YELLOW}./docs/${NC} - Legacy Flutter documentation"
echo -e "  📁 ${YELLOW}./.augment/${NC} - Augment AI configuration"
echo -e "  📁 ${YELLOW}./.augment/rules/${NC} - AI agent rules and guidelines"
echo -e "  📄 ${YELLOW}./.augment/rules/README.md${NC} - Documentation index"
echo ""
echo -e "${BLUE}Quick start:${NC}"
echo -e "  • Read documentation index: ${YELLOW}./.augment/rules/README.md${NC}"
echo -e "  • Core principles: ${YELLOW}./.augment/rules/core-principles.md${NC}"
echo -e "  • State management: ${YELLOW}./.augment/rules/state-management-guide.md${NC}"
echo -e "  • Testing guide: ${YELLOW}./.augment/rules/testing-guide.md${NC}"
echo -e "  • Update docs: ${YELLOW}cd /opt/flutter/flutter-docs && git pull${NC}"
